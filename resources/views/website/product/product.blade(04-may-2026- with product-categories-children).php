...
.category-list .expand-icon.rotated {
...
.category-list .sub-list.expanded {
    ...
    <div class="container">
        ...
        <div class="d-flex d-lg-none overflow-x-auto mb-4 pb-2">
            <div class="d-flex flex-nowrap gap-2">
                <a href="{{ route('shop') }}" class="btn rounded-pill text-nowrap {{ !isset($slug) || !$slug ? 'btn-danger' : 'btn-outline-secondary' }}" style="{{ !isset($slug) || !$slug ? 'background-color: #E10B17; border-color: #e8362a; color: white' : '' }}">
                    {{ __('All Categories') }}
                </a>
                    ...
                    <a href="{{ route('productCategory', $catBtn->slug) }}"
                       ...
                       style="{{ isset($slug) && $slug == $catBtn->slug ? 'background-color: #E10B17; border-color: #e8362a; color: white' : '' }}">
                        ...
                        <span>
                            ...
                            <small class="ms-1">({{ $catBtn->total_product_count ?? 0 }})</small>
                        </span>
                    </a>
                @endforeach
            </div>
                ...
                <div class="sidebar-section">
                    ...
                    <ul class="category-list">
                        <li>
                            <a href="{{ route('shop') }}" class="{{ !isset($slug) || !$slug ? 'active' : '' }}">
                                <div class="category-content">
                                    <span class="category-text">{{ __('All') }}</span>
                        ...
                        @foreach ($allRootCategories as $cat)
                            <li>
                                <a href="{{ route('productCategory', $cat->slug) }}" class="category-link {{ isset($slug) && $slug == $cat->slug ? 'active' : '' }}" data-has-children="{{ $cat->children->isNotEmpty() ? 'true' : 'false' }}">
                                    <div class="category-content">
                                        <span class="category-text">{{ $locale === 'bn' ? $cat->name_bn : $cat->name_en }}</span>
                                        <span class="count">{{ $cat->total_product_count ?? 0 }}</span>
                                    </div>
                                ...
                                @if($cat->children->isNotEmpty())
                                    <ul class="category-list sub-list">
                                        @foreach ($cat->children as $subcategory)
                                            <li>
                                                <a href="{{ route('productCategory', $subcategory->slug) }}" class="category-link {{ isset($slug) && $slug == $subcategory->slug ? 'active' : '' }}" data-has-children="{{ $subcategory->children->isNotEmpty() ? 'true' : 'false' }}">
                                                    <div class="category-content">
                                                        <span class="category-text">{{ $locale === 'bn' ? $subcategory->name_bn : $subcategory->name_en }}</span>
                                                        <span class="count">{{ $subcategory->total_product_count ?? 0 }}</span>
                                                    </div>
                                                ...
                                                @if($subcategory->children->isNotEmpty())
                                                    <ul class="category-list sub-list">
                                                        @foreach ($subcategory->children as $childCategory)
                                                            <li>
                                                                <a href="{{ route('productCategory', $childCategory->slug) }}" class="category-link {{ isset($slug) && $slug == $childCategory->slug ? 'active' : '' }}">
                                                                    <div class="category-content">
                                                                        <span class="category-text">{{ $locale === 'bn' ? $childCategory->name_bn : $childCategory->name_en }}</span>
                                                                        <span class="count">{{ $childCategory->product_count ?? 0 }}</span>
                                                                    </div>
                                                                </a>
                                                            </li>
                                                        @endforeach
...
document.addEventListener('DOMContentLoaded', function() {
    const categoryLinks = document.querySelectorAll('.category-link[data-has-children="true"]');
    ...
    categoryLinks.forEach(link => {
        link.addEventListener('click', function(e) {
            ...
            if (subList && subList.classList.contains('sub-list')) {
                ...
                // Toggle rotated class for icon
                if (expandIcon) {
                    expandIcon.classList.toggle('rotated');
                }

                // If this is a navigation click (not just expand/collapse)
                if (e.target.closest('.category-content') || e.target === this) {
    ...
    // Auto-expand categories that are currently active
    const activeLinks = document.querySelectorAll('.category-link.active');
    activeLinks.forEach(activeLink => {
        let parent = activeLink.closest('.sub-list');
        while (parent) {
            parent.classList.add('expanded');
            const prevLink = parent.previousElementSibling;
            if (prevLink && prevLink.classList.contains('category-link')) {
                const icon = prevLink.querySelector('.expand-icon');
                if (icon) {
