...
class FrontendController extends Controller
{
   ...
   public function selectGetDoctor(Request $request){
    ...
    public function codOrderStore(Request $request)
    {
        ...
        $paymentMethod = $request->input('temp_payment_method');
        $orderNote = $request->order_note ?? null;

        if (Auth::check()) {
            $user = auth()->user();
            if ($request->has('temp_billing_address')) {
                DeliveryLocation::updateOrCreate(
                    ['user_id' => $user->id],
                    [
                        'address_title' => $request->input('temp_billing_address'),
                        'name' => $request->input('temp_name'),
                        'mobile' => $request->input('temp_mobile'),
            ...
            $request->validate([
                ...
                'temp_mobile' => 'required|string|max:20',
                'temp_billing_address' => 'required|string|max:1000',
                // 'email' => 'required|email|max:255',
            ...
            $location->name = $request->input('temp_name');
            $location->email = $request->input('temp_email');
            $location->mobile = $request->input('temp_mobile');
            $location->address_title = $request->input('temp_billing_address');
